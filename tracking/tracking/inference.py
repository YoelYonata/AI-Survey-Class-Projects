...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        reweighting based on observation. If tis happens, resample particles
        ...
        if noisyDistance == None:
            ...
            for pos in legalPos:
                ...
                if emissionModel[dist] > 0:
                    beliefs[pos] = oldBeliefs[pos] * emissionModel[dist]
            #weights are zero
            if not any(beliefs.values()): 
                self.initializeUniformly(gameState)
            #resample
            else:
                temp = []
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        numberOfPermutations = len(permutations)
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            # for ghosts that are captured, move particles to jail
            for i in deadGhost:
            ...
            for ghost in range(self.numGhosts):
                if ghost not in deadGhost:
                    ...
                    prob = prob * model[util.manhattanDistance(pacmanPosition, particle[ghost])]

            particleCounter[particle] += prob
        ...
        if particleCounter.totalCount() == 0:
            ...
            for j in range(len(self.particles)):
                self.particles[j] = util.sample(self.beliefs)       
