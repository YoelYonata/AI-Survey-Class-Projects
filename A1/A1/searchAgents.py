        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if nextPos in state[1]: 
                    nextGoals.remove(nextPos) 
                
                successors.append(((nextPos, nextGoals) , action, 1))
...
def cornersHeuristic(state, problem):
    ...
    while len(goals) > 0 :
        closestGoal = goals [0]
        minDistance = util.manhattanDistance(currentState,goals[0])

        for g in goals:
            d = util.manhattanDistance(currentState,g)
            if d < minDistance:
                closestGoal = g
                minDistance = d

        currentState = closestGoal
